...
class Server {
  ...
  public Optional<Customer> getCustomerCopy(Optional<Customer> customer) {
    return customer.isEmpty() ? Optional.empty() : Optional.of(customer.get().copy());
  }

  public Server copy() {
      return new Server(this.id, getCustomerCopy(this.currentCustomer), getCustomerCopy(waitingCustomer));
  }
  ...
  public Server serve(Customer customer) {
    if (hasWaitingCustomer() && customer.equals(this.waitingCustomer.get())) {
